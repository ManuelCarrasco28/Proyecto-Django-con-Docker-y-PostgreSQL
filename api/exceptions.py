...
def encomiendas_exception_handler(exc, context):
    ...
    """
    # Primero procesar con el handler por defecto de DRF para obtener la respuesta inicial
    response = exception_handler(exc, context)

    if response is not None:
        # Determinar el código semántico y mensaje según el status HTTP
        ...
        if response.status_code == status.HTTP_400_BAD_REQUEST:
            ...
            error_code = 'YA_ENTREGADA'
            message = 'El estado actual del recurso entra en conflicto con esta acción.'

        # Reestructurar el cuerpo de la respuesta
        response.data = {
            'error': True,
            'code': error_code,
            'message': message,
    ...
    # Si la respuesta es None, significa que es un error no controlado (500)
    logger.error(
